...
class ItemViewModel(private val itemId: String?, private val itemRepository: ItemRepository) :
    ...
    fun saveOrUpdateItem(date: String, boolean: Boolean, number: Int, text: String, lat:Double, lng:Double) {
        viewModelScope.launch {
            Log.d(TAG, "saveOrUpdateItem...");
            try {
                uiState = uiState.copy(isSaving = true, savingError = null)
                val item = uiState.item?.copy(date=date, boolean = boolean, number = number, text = text, lat=lat, lng=lng)
                if (itemId == null) {
